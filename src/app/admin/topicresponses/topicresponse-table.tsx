...
export function DataTableToolbar<TData>({ table, topics }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex items-center gap-1 dark:text-white">
          ...
          <Input className="max-w-xs" placeholder="respuestaPlanteo filter..."
              value={(table.getColumn("respuestaPlanteo")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("respuestaPlanteo")?.setFilterValue(event.target.value)}                
          />
          
      
          <Input className="max-w-xs" placeholder="respuestaSolucion filter..."
              value={(table.getColumn("respuestaSolucion")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("respuestaSolucion")?.setFilterValue(event.target.value)}                
          />
