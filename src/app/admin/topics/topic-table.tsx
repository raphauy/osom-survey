...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex items-center gap-1 dark:text-white">
          ...
          <Input className="max-w-xs" placeholder="prompt filter..."
              value={(table.getColumn("prompt")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("prompt")?.setFilterValue(event.target.value)}                
          />
          
          
        {table.getColumn("enabledStr") && (
          <DataTableFacetedFilter
            column={table.getColumn("enabledStr")}
            title="Activo"
            options={["si", "no"]}
          />
        )}
